    ...
    <Routes>
      ...
      <Fragment>
        {tabMenus.map((menu, index) => (
          <Fragment key={index}>
            {menu["Sub Menu"] &&
              menu["Sub Menu"].map((submenu, idx) => (
                <Fragment key={idx}>
                  <Route path={`/${toPathname(menu["Menu Utama"].menu_name)}/${toPathname(submenu.submenu_name)}`} element={<DashboardSlugPage parent={menu["Menu Utama"].menu_name} slug={submenu.submenu_name} />} />
                  <Route path={`/${toPathname(menu["Menu Utama"].menu_name)}/${toPathname(submenu.submenu_name)}/:params`} element={<DashboardParamsPage parent={menu["Menu Utama"].menu_name} slug={submenu.submenu_name} />} />
                </Fragment>
              ))}
          </Fragment>
        ))}
